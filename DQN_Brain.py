...
class DeepQNetwork:
    def __init__(
        ...
        # initialize zero memory [s, a, r, s_]  #state size:n_features action size:1 reward size:1 state_ size:n_features
        ...
        #load model
        self.saver = tf.train.import_meta_graph('./DQN/ckpt/model.ckpt-18000000.meta')
        self.saver.restore(self.sess,tf.train.latest_checkpoint('./DQN/ckpt'))
        #self.sess.run(tf.global_variables_initializer())
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('eval_net'):
            # c_names(collections_names) are the collections to store variables     #\表示沒有[],（）
            ...
            with tf.variable_scope('l6'):
                ...
                self.q_eval = tf.matmul(l5, w6) + b6
        ...
        with tf.variable_scope('target_net'):
            ...
            with tf.variable_scope('l6'):
                w6 = tf.get_variable('w6', [n_l5, self.n_actions], initializer=w_initializer, collections=c_names)
                b6 = tf.get_variable('b6', [1, self.n_actions], initializer=b_initializer, collections=c_names)
                self.q_next = tf.matmul(l5, w6) + b6
...
#RL.store_transition(observation, action, reward, observation_)
    def store_transition(self, s, a, r, s_):
        ...
        if (self.memory_counter >=20000)and (self.memory_counter %20000==0):
            self.saver.save(self.sess,"./DQN/ckpt/model.ckpt",global_step = self.memory_counter)
    ...
    def learn(self):
        ...
        eval_act_index = batch_memory[:, self.n_features].astype(int)   #get action column
        reward = batch_memory[:, self.n_features + 1]   #get reward column

        

        q_target[batch_index, eval_act_index] = reward + self.gamma * np.max(q_next, axis=1)    # np.max(q_next, axis=1) get the max value form batch_size * n_action
                                                                                                #   return batch_size * 1
