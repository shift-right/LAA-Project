...
class DuelingDQN:
    def __init__(
        ...
        #load model
        self.saver = tf.train.import_meta_graph('./DuelingDQN/ckpt/model.ckpt-18000000.meta')
        self.saver.restore(self.sess,tf.train.latest_checkpoint('./DuelingDQN/ckpt'))
        #self.sess.run(tf.global_variables_initializer())
        ...
        w6 = graph.get_tensor_by_name("eval_net/Value/w6:0")
        b6 = graph.get_tensor_by_name("eval_net/Value/b6:0")
        self.V = graph.get_tensor_by_name("eval_net/Value/add:0")
        w6 = graph.get_tensor_by_name("eval_net/Advantage/w6:0")
        b6 = graph.get_tensor_by_name("eval_net/Advantage/b6:0")
        self.A = graph.get_tensor_by_name("eval_net/Advantage/add:0")
        ...
        w6 = graph.get_tensor_by_name("target_net/Value/w6:0")
        b6 = graph.get_tensor_by_name("target_net/Value/b6:0")
        self.V = graph.get_tensor_by_name("target_net/Value/add:0")
        w6 = graph.get_tensor_by_name("target_net/Advantage/w6:0")
        b6 = graph.get_tensor_by_name("target_net/Advantage/b6:0")
        self.A = graph.get_tensor_by_name("target_net/Advantage/add:0")
        ...
        self.cost_his = []
        #self.saver.save(self.sess,"./DuelingDQN/ckpt/model.ckpt")
        ...
        def build_layers(s, c_names, n_l1, n_l2, n_l3,n_l4, n_l5, w_initializer, b_initializer):
            ...
            if self.dueling:
                ...
                with tf.variable_scope('Value'):
                    w6 = tf.get_variable('w6', [n_l5, 1], initializer=w_initializer, collections=c_names)
    ...
    def store_transition(self, s, a, r, s_):
        ...
        if (self.memory_counter >=20000)and (self.memory_counter %20000==0):
            self.saver.save(self.sess,"./DuelingDQN/ckpt/model.ckpt",global_step = self.memory_counter)
